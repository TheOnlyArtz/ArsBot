...
exports.run = (client, message) => {
  ...
  if (!platform) return message.reply("You must enter a platform example: `google / bitly`");
  if (platform !== "google" && platform !== "bitly") return message.reply("Please enter a valid short link from `google / bitly`")
  if (!unshortLink) return message.reply("You must give a shorten link so the bot can fetch the information\n and the originl LINK!");
  if (!unshortLink.includes("https://goo.gl") && !unshortLink.includes("http://bit.ly")) return message.reply(`Did not detect a ${platform} link!`)
