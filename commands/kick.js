...
exports.run = (client, message, args) => {
  if(!message.guild.member(message.author).hasPermission("KICK_MEMBERS")) return message.reply(":lock: You dont have permissions for that").catch(console.error);
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply(":lock: **I** need `KICK_MEMBERS` Permissions to execute `mute`").catch(console.error);
  ...
  if(message.mentions.users.size < 1) return message.reply("You need to mention someone to Kick him!. **Usage:**\`~kick [@mention] [example]\`");
  if(!reason) return message.reply("You must give me a reason for mute **Usage:**\`~kick [@mention] [example]\`");
  if(!message.guild.member(user).kickable) return message.reply("This member is above me in the `role chain` Can't kick him")
