  ...
  if (message.content == "~weed upgrade menu" || message.content == "~weed upgrading menu" || message.content == "~weed upgrade") {
    knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.guild.ownerID).then(l => {
      knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.author.id).then(row => {
        if(!l[0]) return message.reply("Econ mod is not enabled for this server!")
        if(l[0].agree == "false") return message.reply("Econ mod is not enabled for this server!")
...
if (message.content == "~weed upgrade lemonHaze" || message.content == "~weed upgrade lemonhaze"|| message.content == "~weed upgrade lemon haze") {
  knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.guild.ownerID).then(async(l) => {
    if(!l) return message.reply("Econ mod is not enabled for this server!")
    knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.author.id).then(async(row) => {
      if (!row[0]) {
        await message.reply("You cant upgrade since you dont even have weed **YET**")
  ...
  if (message.content.startsWith("~weed bank")) {
    knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.guild.ownerID).then(async(l) => {
      knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.author.id).then(async(row) => {
        if(!l[0]) return message.reply("Econ mod is not enabled for this server!")
        if (`${l[0].agree}` === "false") return message.reply("Econ mode is not enabled for this server!")
        if (!row[0]) {
    ...
    knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.guild.ownerID).then(async(l) => {
      knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.author.id).then(async(row) => {
        ...
        await message.channel.send(`${message.author.username} Bought a cigarette -15 grams of weed`).then(msg => {
            ...
            setTimeout(() => { msg.edit('🚬 ☁☁ ') }, 1500);
            setTimeout(() => { msg.edit('🚬 ☁☁☁ ') }, 2000);
            setTimeout(() => { msg.edit('🚬 ☁☁') }, 2500);
            setTimeout(() => { msg.edit('🚬 ☁') }, 3000);
            setTimeout(() => { msg.edit('🚬 ') }, 3500);
            setTimeout(() => { msg.edit(`${message.author.username} Finished smoking`) }, 1000)
          })
          knexDB.update({
            userid: message.author.id,
            guildid: message.guild.id,
    ...
    knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.guild.ownerID).then(async(l) => {
      knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.author.id).then(async(row) => {
        ...
        if (!row[0]) {
          ...
          if (!guess) return message.reply("Put a guessing number to bet on please! \`EXAMPLE: ~betweed <1-5> <deposit amount>\`")
          if (isNaN(guess)) return message.reply("You must enter an actual guessing number \`EXAMPLE: ~betweed <guessnumber> <deposit amount>\`")
          if (Number(guess) > 5) return message.reply("The number of the guess must be lower than 5. \`EXAMPLE: ~betweed <guessnumber> <deposit amount>\`");
          if (Number(guess) < 1) return message.reply("The number of the guess must be greater than 1. \`EXAMPLE: ~betweed <guessnumber> <deposit amount>\`");
          //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          if (!deposit) return message.reply(`Put some weed grams to bet on please! avaliable weed: ${row[0].weedamount} Grams \`EXAMPLE: ~betweed <guessnumber> <deposit amount>\``)
          if (deposit > row[0].weedamount) return message.reply(`You dont have ${deposit} Grams of weed`)
          if (isNaN(deposit)) return message.reply("You must enter an actual number of weed \`EXAMPLE: ~betweed <guessnumber> <deposit amount>\`")
          if (Number(guess) !== final) {
            const embed = new Discord.RichEmbed()
            .addField("Bet Info:", `**Amount:** ${deposit} Grams\n**Multiplier:** 5x\n*BetStatus:* **[LOST]**\n**To The Bank:** **-**${deposit} Grams of weed!`)
            await message.channel.send({embed : embed}).then(m => m.react("❌")).catch(console.error)
            knexDB.update({
  ...
  if(message.content.startsWith("~weed sell")) {
    knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.guild.ownerID).then(async(owner) => {
      ...
      if(!owner[0]) return message.reply("Econ mode is not enabled for this server");
      if(owner[0].agree == "false") return message.reply("Econ mode is not enabled for this server");
      ...
      knexDB.from('weedbank').where('guildid', message.guild.id).andWhere('userid', message.author.id).then(async(row) => {
        ...
        if(!row[0]) return message.reply("We have opened you a bank account");
        if(!amount) return message.reply("You must type weed amount to sell");
        if (amount > row[0].weedamount) return message.reply(`You dont have that much weed, avaliable weed: ${row[0].weedamount} (grams)`)
        knexDB.update({
